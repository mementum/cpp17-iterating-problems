...
We started this new chapter because iterating over the list of types was not the goal. That is why we now have solutions based on iterators. Unfortunately, these solutions rely on knowledge of the `std::variant` and the containers with helpers to fetch types from `std::cin` and put them to `std::out`.
...
The trick is therefore to specialize a `std::false` / `std::true` check, where the true part is a specialization containing an `std::variant<Args...>`.
