...
After increasing `a` with `a++`, the boolean value of the comparison `a < b` is what will determine if the `while` loop continues or not. But with a *Fold Expression*, this is different:
...
Recursion is managed by the *Fold Expression* with the `,` operator. We have seen ourselves in the need to outsource the processing of the type again to the `in_out` function. And we are in the business of less is more.
...
And here is the complete single method approach with a *folding expression*.
