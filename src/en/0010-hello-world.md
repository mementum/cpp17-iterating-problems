...
But because our `operator *()` and `operator ++()` are *NOP* implementation returning a reference to itself, creating a `tmp` copy is not needed and we can implement `++(int)` also as an *NOP*.
