...
Remember we said that one of the reasons to use the problem sets available at *HackerRank* was the fact that it seemed partially abandoned. The editor was therefore not locked and we could redo the entire solution including the input and output, parts that seem sometimes written by very motivated but inexperienced coders.
...
Your humble author may not be the best coder in the world, but he for sure knows something: *Variable Length Arrays* (aka *VLA*) are not part of the *C++* standard.

Furthermore, and unlike stated, the *VLA* functionality is part of the *C* standard. It has been part of it for many years. The question is if the statement (at least for *C++*) can be proven to be true. Let us give it a try.

```cpp title
--8<-- "{sourcedir}/08-arrays-intro/arrays-intro-01.cpp"
```

Blistering Barnacles! It does work. Notice how we have marked the *VLA* line with the following comment: *"NON-STANDARD C++ - Variable Length Array Extension"*. Because it is a compiler extension. Originally only *GCC* supported it, but it is also supported by *clang* today. It is now clear how the author came to the conclusion that *C++* supports *Variable Length Arrays*: he/she gave it a try and it simply worked. This may (will) break with other compilers, of course.
...
One of the pleasures of *Modern C++*, anything *C++11* and newer is that language designers understood that new facilities were needed to ease the burden on coders. Someone asked the rhetoric question about manual allocation/deallocation of memory for pointers/arrays and why that could not be managed with intelligence. And `std::unique_ptr<T>` was born (there is a `Deleter` template parameter that I am omitting there). As it happened with the `_v` variants of compile time checks, that help people, someone thought that no matter how good our `std::unique_ptr` was, a helper would be even better. *C++14* brought `std::make_unique`.
...
Even better: we can use that pointer to get a *reversed* iterator with `std::make_reverse_iterator` to traverse the array backwards as requested by the problem description. When iterators launched they were touted as glorified pointers and seeing that constructions we probably want to agree.
...
Although we still are going to use `N`, we are going to move to use `std::copy`. First by reading integers until we hit the end of the input stream. Nothing fancy here.
...
We are getting closer to a general solution. `abegin` (for *"array begin"*) gives us the proper abstraction to understand we can get a position within the array with `std::next`. The last one obviously by moving the pseudo-begin by `N` positions. If you are wondering why `std::begin` is not being used here the answer is simple: it does not work. The official reason being that the end of the array (pointer-based) cannot be known and therefore `std::end` cannot be calculated to have a `begin` to `end` traversal.
...
Let us remove all those restrictions. For that we are going to need the following things:
  ...
  - Compile time branch selection by checking the properties of the iterators and choosing the appropriate way to work with a container to hold the input values if needed be.
...
For the sake of checking things under a different light, we will be checking for subclassing in the case of `std::stack<T>`. This container adaptor is actually the perfect fit for the problem description: we push things into it and the retrieval happens always in `LIFO` order. Notice that it is a *container adaptor* and not a container. It relies on other containers to provide the functionality. It does feature a different interface and we would have needed to check three method names: `push`, `top` and `pop`. But this does also work for us.
...
```
Notice how the *C++17* beast `if constexpr`, let us formulate in an imperative and very natural form how we perform the selection of the different scenarios. 


If the container is not our *LIFO* container, we check what insertion method is supported. Some iterators support all three, `push_back`, `push_front` and `insert` whereas others do only support the latter. Choosing to check `push_front` first means that we can output in reverse order by traversing the container from the beginning to the end, without reversing iterators. The other options will force us to use `rbegin` and `rend`, because the elements will be pushed at the back of the container.

Notice that we have added some informational messages instantiating an output iterator using `std::cerr`. This is because we can choose how to build this solution by defining `CASE0 => CASE3` or letting the defaults be build.

Doing `make 06 case2`, for example, will using the standard input iterators and an `std::list`. This will in turn select a `push_front` approach and that will be shown with the message being pushed over the standard error stream. Like this.
...
Choosing `case0` will first store the values in an `std::vector` to the *Bidirectional Iterators* case play along. No container will be instantiated inside the solution function.
