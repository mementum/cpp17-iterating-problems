...
# For mkdocs targets: numbering that makes the cut
SRC_MK_0 := 0007
SRC_MK_9 := 7000
MK_TAB_MAIN := "Book"
...
# Languages not ending in "-" are "enabled" and will be considered for a buld
BLANGS := $(shell echo $(MAYBEBLANGS) | tr ' ' '\n' | grep -E '.+[^-]$$')
# Languages ending in "-" are "disabled" and will not be considered for a buld
...
# BUILDCHAPS := $(shell echo $(SRCFILES) | tr ' ' '\n' | grep -E '$(buildchap)')
...
# document covers
BACKCOVER_TITLE := $(shell grep :backcover-title: $(ADOC_HEADER) | cut -d: -f3)
...
BNAME := $(shell head -n 1 $(ADOC_HEADER))
# extra $ to quote the EOLD $
# strip leading "= ", "remove everything after " : " until EOLD
BNAME := $(shell echo "$(BNAME)" | sed 's/^=[ \t]*//;s/ : [ \t]*.*$$//')
...
MKDOCS_INDEX_MD := $(firstword $(BOOK_MKDOCS))
BUILD_MKDOCS_INDEX_MD := $(BUILD_DIR_MKDOCS)/index.md
...
define mkdocs_navigation =
	$(eval tgts := $(notdir $(1)))
	$(eval tgidx := $(firstword $(tgts)))
	$(eval tgts := $(subst $(tgidx),index.md,$(tgts)))
...
$(BUILD_MKDOCS_INDEX_MD): $(MKDOCS_INDEX_MD)
	$(call makedir_if,$@)
	cp $< $@
...
# $(BUILD_DIR_MKDOCS)/%.md: $(ADOC_HEADERS) $(SRC_LANG)/%.md $(wildcard $(SRC_00)/%.md) $(wildcard $(SRC_ZZ)/%.md)
# $(BUILD_DIR_MKDOCS)/%.md: $(ADOC_HEADERS) $(foreach x,$(SRC_DIRS),%$(x).md)
$(BUILD_DIR_MKDOCS)/%.md: $(ADOC_HEADERS) $(SRC_LANG)/%.md
	...
	cat $(mdfiles) >> $(tmpfile)
ifeq ($(GREP_OUT_MK),)
	$(FIX_MD2MK) -o $@ $(ADOC_HEADERS) $(tmpfile)
else
	$(call grep_ev,$(FIX_MD2MK) -lo -o $@ $(ADOC_HEADERS) $(tmpfile),$(GREP_OUT_MK))
endif
	-rm -f $(tmpfile)

# $(SRC_LANG)/%.md: $(SRC_00)/%.md $(SRC_ZZ)/%.md
...
ifeq ($(GREP_OUT_MD),)
	...
	$(call grep_ev,$(FIX_MD2AD) -lo -o $(tmpfile) $^,$(GREP_OUT_MD))
endif
	$(KRAMDOC) $(KRAMDOC_OPTS) --output=$@ $(tmpfile)
	-rm -f $(tmpfile)
ifeq ($(GREP_OUT_AD),)
	$(FIX_AD2AD) -i $@
else
	$(call grep_ev,$(FIX_AD2AD) -lo -i $@,$(GREP_OUT_AD))
endif
...
define make_pdf =
	...
	$(ADOCTORPDF) \
		$(basename $(ADOCTOR_DSTFILE))$(DEV)$(suffix $(ADOCTOR_DSTFILE)) \
		-a pdf-theme=$(4) \
		$(ADOCTOR_PDF_FONTS) \
		$(ADOCTOR_PDF_EXTENDERS) \
		$(ADOCTOR_IMGDIR) \
		$(2) \
		$(3) \
		$(ADOC_PDF_EXTRA_OPTS) \
...
define make_epub =
	...
	$(ADOCTOREPUB) \
		$(ADOCTOR_DSTFILE_CURDIR) \
		$(2) \
		$<

	$(call makedir_if_and_move,$@)
...
help:
	...
	@echo "  mkdocs-build (Publish and build the mkdocs site docs)"
	@echo "     (also mk-serve, mk-kill, mk-build)"
	...
	@echo "  touch (ensure rebuild even if intermediates unchanged)"
