...
class Fixer:
    ...
    def proc_line(self, proc: Any, line: str) -> str | list[str]:
        ...
        lmatch_re = lsub_re = lproc_re = EMPTY_STR
        ...
        if lmatch := getattr(proc, LMATCH, EMPTY_STR):
            if debug:
                linfo(f"{lmatch = }")
        ...
        if CALLATTR in dir(proc):
            if not lmatch_re:  # no match ... record if sub_re requested
                lproc_re = lsub_re
    ...
    def process_file(self, lineit: Iterable) -> None:
    ...
    def call_line_block(
        ...
        *args,
        ...
        if  "lmatch" in kw:
            if lproc_re:
                kw["lmatch"] = self.lmatch = re.match(lproc_re, lorig)

            if self.lmatch is None:
                return line
